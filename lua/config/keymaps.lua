...
keymap("n", "vm", function()
  ...
  if is_tree_buf then
    local image_path = require("nvim-tree.api").tree.get_node_under_cursor().absolute_path
  else
    local parent_path = tree.get_node_under_cursor().parent.absolute_path
    local rel_img_path = grabTextInsideParentheses()
    -- the yank way
    -- local rel_img_path = utils.get_register_value { sanitize = true }
    image_path = parent_path .. "/" .. rel_img_path
  end

  vim.fn.system("gwenview " .. image_path)
end, { expr = true })
...
function grabTextInsideParentheses()
  ...
  -- Find the positions of the surrounding parentheses
  local start_pos, end_pos = line:find "%b()"
  if not start_pos or not end_pos then
    print "No parentheses found on line"
    return
  end
  local text_inside = ""
  -- Check if cursor is inside parentheses
  if cur_col >= start_pos and cur_col <= end_pos then
    -- Extract text inside parentheses
    text_inside = line:sub(start_pos + 1, end_pos - 1)

    -- Grep the extracted text (or you can modify to do whatever you want with the text)
    -- print("Text inside parentheses: " .. text_inside)
    -- Here you can add the code to handle the text, like grepping or opening the file, etc.
  end
  return text_inside
end
